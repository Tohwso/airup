...
4. **Run the Local Verification Gate** (see below) BEFORE any agent-level audit
5. **Parse the AUDIT scope** from the human's request (see AUDIT Scope Parsing below)
6. Route to the appropriate agent(s) based on scope
7. Collect findings from each agent
8. Produce the **Consolidated Audit Report** (see below)

### Local Verification Gate (`verify.sh`)

Before any AUDIT or PR approval, run the local verification gate:

```bash
./verify.sh          # full (compile + unit + E2E + secrets + context + PIT)
./verify.sh --fast   # skip PIT mutation testing
```

The script runs 6 checks and produces a structured JSON verdict at
`target/e2e-evidence/verify-report.json`:

| Step | Check | Blocks PR? |
|------|-------|-----------|
| 1 | Compile (`mvn test-compile`) | YES |
| 2 | Unit tests (all except E2E) | YES |
| 3 | E2E + Contract tests | YES |
| 4 | Secrets scan (gitleaks) | YES |
| 5 | Context load (Spring ApplicationContext starts) | YES |
| 6 | PIT mutation testing | NO (informational) |

**Rules:**
- `VERDICT: GO` → PR can proceed to human review
- `VERDICT: NO-GO` → PR is blocked. Fix the failing check before proceeding.
- If `verify.sh` does not exist in the project, the Governor creates it using the
  E2E Local Test Pattern infrastructure (WireMock, EmbeddedKafka, H2).
- The JSON report is designed for AI consumption (~500 bytes, structured).
  Include it in the Consolidated Audit Report.
- E2E evidence files (`target/e2e-evidence/*.json`) contain per-scenario DB state
  snapshots generated by `E2EEvidenceWatcher`. Reference these in findings.
...
- Checks: compile ✅ | unit_tests ✅ (N tests) | e2e_tests ✅ (N scenarios) | secrets ✅ | context_load ✅ | mutation ⊘
