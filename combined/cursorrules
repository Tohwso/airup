...
- **LIGHT** (< 3 artifacts, no architectural change): Governor updates artifacts directly. Example: append 1 RF to requirements.md.
- **MEDIUM** (3-5 artifacts, or API/domain model change): Governor delegates to each artifact's owner agent for surgical updates.
- **HEAVY** (architectural change, new bounded context): Governor triggers mini-pipeline with only impacted phases.
...
- 0-10%: 🟢 Healthy — 10-30%: 🟡 Alert (batch sync recommended) — >30%: 🔴 Critical (spec unreliable)

---

## Evolution Phase Protocol

After initial pipeline (Elaboration) and task-based development (Construction), the project enters **Evolution** — ongoing maintenance and incremental improvements.
...
2. **Changelog always.** Every code change gets a CL-NNN entry. No exceptions.
...
5. **Spec Drift gate.** If Spec Drift Score > 30%, recommend a sync batch before accepting new feature requests.
